...
class _EvidenceAnalysisScreenState extends State<EvidenceAnalysisScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final panelUnlocked = engine.isPanelUnlocked(widget.panelId);
            ...
            child: NeonContainer(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'This evidence panel is locked. Complete the linked mini-game to unlock it.',
                      style: TextStyle(
                        color: CyberColors.neonAmber,
                        fontSize: 13,
                        height: 1.5,
...
class _EvidenceContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (panel.evidenceType) {
      case 'chat':
        return _ChatThread(item: item, allItems: allItems);
      case 'files':
        return _FileDocument(item: item);
      case 'meta':
      case 'ip':
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: item.rows.asMap().entries.map((entry) {
                final i = entry.key;
...
class _MiniGameUnlockPreview extends StatelessWidget {
  final CaseEngine engine;
  final EvidencePanel panel;
  final MinigameConfig minigame;
  const _MiniGameUnlockPreview({
  ...
  Widget build(BuildContext context) {
    final onSuccess =
        minigame.rawJson['onSuccess'] as Map<String, dynamic>? ?? const {};
    final unlockPanelId = onSuccess['unlocksPanel'] as String?;
    final unlockItemId = onSuccess['unlocksHiddenItem'] as String?;

    final rewards = <String>[];

    if (unlockPanelId != null && unlockPanelId.isNotEmpty) {
      final unlockedPanel = engine.caseFile.panelById(unlockPanelId);
      rewards.add('Panel unlocked: ${unlockedPanel?.label ?? unlockPanelId}');
    }

    if (unlockItemId != null && unlockItemId.isNotEmpty) {
      final directItem = panel.hiddenItem;
      if (directItem != null && directItem.id == unlockItemId) {
        rewards.add('Evidence unlocked: ${directItem.label}');
      } else {
        for (final p in engine.caseFile.evidencePanels) {
          final hidden = p.hiddenItem;
          if (hidden != null && hidden.id == unlockItemId) {
            rewards.add('Evidence unlocked: ${hidden.label}');
            break;
          }
        }
      }
    ...
    // Also detect hidden evidence linked by unlockedByMinigame in this panel.
    final linkedHidden = panel.hiddenItem;
    if (linkedHidden != null &&
        linkedHidden.unlockedByMinigameId == minigame.id &&
        !rewards.any((r) => r.contains(linkedHidden.label))) {
      rewards.add('Evidence unlocked: ${linkedHidden.label}');
    }
